    ...
    for s in steps[:6]:
        if s["units_late"] >= 2 and s["avg_days"] >= 2:
            ...
            out.append({
                ...
                "action": "Buka Template Jadwal → ubah hari mulai/selesai langkah ini.",
            })
        ...
        if s["wait_days"] and s["avg_days"] >= s["wait_days"]:
            out.append({
                ...
                "templates": s["templates"],
                "title": f"Tinjau waktu tunggu setelah {s['step_code']}",
                "detail": (f"Waktu tunggu {s['wait_days']} hari pada '{s['name']}' "
                           f"membuat pekerjaan berikutnya menunggu, sementara langkah ini "
                           f"sendiri telat rata-rata {s['avg_days']} hari."),
                "action": "Sesuaikan waktu tunggu/curing pada template agar realistis.",
            })
    for p in people[:3]:
    ...
    for t in types[:2]:
        if t["items_late"] >= 4 and t["late_rate"] >= 35:
            out.append({
                ...
                "action": "Duplikat template tipe ini lalu sesuaikan durasi keseluruhan.",
            })
    return out[:10]
