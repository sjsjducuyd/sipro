...
def is_workday(day: date, work_days_per_week: int, holidays: set) -> bool:
...
def next_workday(from_day: date, wdpw: int, holidays: set) -> date:
    day = from_day
    for _ in range(60):
        if is_workday(day, wdpw, holidays):
            return day
...
async def generate_schedule(org: str, unit: dict, tpl: dict, start_date: str, actor: str,
    ...
    start = next_workday(_d(start_date), wdpw, holidays) if mode == "working_days" \
        else _d(start_date)
    ...
    finish = date_for_day(start, last_day, mode, wdpw, holidays)
    roles = {}
    docs = []
    for order, s in enumerate(steps, start=1):
        ...
        p_start = date_for_day(start, int(s.get("day_from") or 1), mode, wdpw, holidays)
...
async def recompute_schedule(org: str, schedule_id: str) -> dict:
    ...
    cstatus = {"not_started": "not_started", "in_progress": "in_progress",
               "at_risk": "in_progress", "on_hold": "in_progress", "done": "done"}[status]
