...
async def ensure_indexes():
    ...
    await db.build_bulk_runs.create_index([("org_id", 1), ("kind", 1), ("client_ref", 1)],
...
def plan_for_template(tpl: dict, start_date: str) -> dict:
    ...
    start = be.next_workday(be._d(start_date), wdpw, holidays) if mode == "working_days" \
        else be._d(start_date)
    last = max(int(s.get("day_to") or s.get("day_from") or 1) for s in steps)
    finish = be.date_for_day(start, last, mode, wdpw, holidays)
...
async def candidates(org: str, project_id: str = None, block: str = None,
    ...
    for u in rows:
        ...
        if isinstance(tpl, str):
            ...
            continue
        try:
            plan = plan_for_template(tpl, today)
        except ValueError as e:
            row["reason"] = str(e)
            out.append(row)
            continue
...
async def plan_create(org: str, unit_ids: list, template_id: str = None,
    ...
    for idx, u in enumerate(ordered):
        ...
        try:
            ...
            plan = plan_for_template(tpl, raw_start)
        except ValueError as e:
            row["reason"] = str(e)
            rows.append(row)
            continue
...
async def plan_shift(org: str, schedule_ids: list, shift_days: int) -> dict:
    ...
    for s in scheds:
        ...
        mode = s.get("calendar_mode", "working_days")
        wdpw = int(s.get("work_days_per_week") or 6)
        holidays = set(s.get("holidays") or [])
        raw = be._d(s.get("start_date")) + timedelta(days=days)
        base = be.next_workday(raw, wdpw, holidays) if mode == "working_days" else raw
        moves = []
        for i in movable:
            ps = be.date_for_day(base, int(i.get("day_from") or 1), mode, wdpw, holidays)
