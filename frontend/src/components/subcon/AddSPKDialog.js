    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5 sm:col-span-2"><Label>Judul Pekerjaan</Label><Input value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="mis. Pekerjaan Struktur Blok A" /></div>
          <div className="space-y-1.5"><Label>Nilai Kontrak (Rp)</Label><Input type="number" value={form.contract_value} onChange={(e) => set("contract_value", e.target.value)} /></div>
          ...
          <div className="space-y-1.5"><Label>Mulai</Label><Input type="date" value={form.start_date} onChange={(e) => set("start_date", e.target.value)} /></div>
          <div className="space-y-1.5"><Label>Selesai</Label><Input type="date" value={form.end_date} onChange={(e) => set("end_date", e.target.value)} /></div>
          <div className="space-y-1.5 sm:col-span-2"><Label>Lingkup</Label><Textarea rows={2} value={form.scope} onChange={(e) => set("scope", e.target.value)} /></div>
        </div>
