      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5"><Label>Judul</Label>
              <Input data-testid={CHANGE_ORDERS.titleInput} value={form.title}
                onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="mis. Tambah pekerjaan tangga" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label>Delta Nilai (Rp)</Label>
                <Input type="number" data-testid={CHANGE_ORDERS.deltaInput} value={form.value_delta}
                  onChange={(e) => setForm((f) => ({ ...f, value_delta: e.target.value }))} placeholder="+/- IDR" />
              </div>
              <div className="space-y-1.5"><Label>Tambah Waktu (hari)</Label>
                <Input type="number" data-testid={CHANGE_ORDERS.extInput} value={form.time_extension_days}
                  onChange={(e) => setForm((f) => ({ ...f, time_extension_days: e.target.value }))} placeholder="0" />
              </div>
            </div>
            <div className="space-y-1.5"><Label>Alasan (opsional)</Label>
