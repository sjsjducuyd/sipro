      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Input data-testid={MASTER.docFormCode} value={form.code} disabled={editing}
              onChange={(e) => set("code", e.target.value.toUpperCase())} placeholder="mis. BAST" />
          </div>
          <div className="space-y-1.5">
            <Label>Nama Template</Label>
            <Input data-testid={MASTER.docFormName} value={form.name}
              onChange={(e) => set("name", e.target.value)} placeholder="mis. Berita Acara Serah Terima" />
          </div>
          <div className="space-y-1.5 sm:col-span-2">
            <Label>Isi Template</Label>
            <Textarea data-testid={MASTER.docFormContent} rows={10} value={form.content}
