...
def audit_guards(fe: str):
    ...
    check('create_index([("org_id", 1), ("kind", 1), ("client_ref", 1)],' in bb
          and "unique=True" in bb and "_prior_run" in bb,
          "INV-34-8 idempotensi klik ganda dijaga index unik + pemutaran ulang")
    ...
    us = (FE / "components" / "construction" / "UnitScheduleSheet.js").read_text(encoding="utf-8")
    check("ShiftHistoryPanel" in us,
          "riwayat penggeseran tampil pada sheet jadwal unit (transparansi)")
    ...
    check("_GROUPS_P34" in (BE / "reference.py").read_text(encoding="utf-8"),
